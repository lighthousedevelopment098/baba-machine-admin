...
const BannerSetup = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (banner) => (
        <img
          src={`${apiConfig.bucket}/${banner?.bannerImage}`}
          alt={banner.name}
  ...
  return (
    <div className="bg-gray-100 px-4 sm:px-6 lg:px-8 py-6 w-full min-h-screen">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold text-gray-800">Manage Banners</h1>
        <ActionButton
          to="/addbannerform"
          className="px-6 py-2 bg-primary-500 hover:bg-primary-dark-500 text-white rounded-md"
          label="Add Banner"
        />
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center text-gray-500 mt-10">
          <p>No banners available. Click "Add Banner" to create one!</p>
        </div>
      ) : (
        <React.Suspense
          fallback={
